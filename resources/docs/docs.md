...
The `home-routes` handler in the `jimw-clj-binding-games.routes.home` namespace
...
The `home-page` function will in turn call the `jimw-clj-binding-games.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/jimw_clj_binding_games/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `jimw-clj-binding-games.handler` namespace:
...
Request middleware functions are located under the `jimw-clj-binding-games.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `jimw-clj-binding-games.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `jimw-clj-binding-games.db.core` in some other namespace.
